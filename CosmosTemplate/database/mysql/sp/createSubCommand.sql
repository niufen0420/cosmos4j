...
DROP PROCEDURE IF EXISTS `cosmos`.`createSubCommand`$$

CREATE
    /*[DEFINER = { user | CURRENT_USER }]*/
    PROCEDURE `cosmos`.`createSubCommand`(IN parentDomain VARCHAR(50), IN parentCommand VARCHAR(100), IN composite VARCHAR(100), IN domain VARCHAR(50), IN name VARCHAR(100), IN command VARCHAR(4000), IN executer VARCHAR(100), IN remark VARCHAR(300), IN commandType VARCHAR(100), IN debugLevel VARCHAR(100))
    /*LANGUAGE SQL
    | [NOT] DETERMINISTIC
