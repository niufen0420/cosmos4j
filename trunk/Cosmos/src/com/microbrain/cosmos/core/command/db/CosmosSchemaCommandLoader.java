 ...
 * Schema型数据库的存储过程本地装载器，用来从数据库系统中装载可供调用的存储过程和函数。
 * </p>
 * 
 ...
 * @since CFDK 1.0
 */
public class CosmosSchemaCommandLoader extends CosmosConfigLocalCommandLoader {
	 ...
	 *            所属数据库域。
	 */
	public CosmosSchemaCommandLoader(CosmosFactory factory,
	...
	public Collection<CosmosCommand> loadAllLocalRootCommands()
		...
		try {
			if (callableExecuter != null) {
				...
				ars = dbmd.getProcedureColumns(null, domain.getSchema(), "%",
						"%");
				...
				rs = dbmd.getProcedures(null, domain.getSchema(), "%");
				try {
					while (rs.next()) {
	...
	public CosmosCommand loadLocalRootCommand(String name)
		...
		try {
			if (callableExecuter != null) {
				conn = domain.getConnection();
				dbmd = conn.getMetaData();
				ars = dbmd.getProcedureColumns(null, domain.getSchema(), name,
						"%");
				...
				rs = dbmd.getProcedures(null, domain.getSchema(), name);
				try {
					if (rs.next()) {
						short typeValue = rs.getShort(8);
						CallableType type = null;
