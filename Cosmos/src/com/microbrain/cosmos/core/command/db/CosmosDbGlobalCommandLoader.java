...
/**
 * <p>
 * <code>CosmosDbGlobalCommandLoader</code> 是一个基于数据库的全局命令装载器，通过将命令放置在数据库中，
 * 开发人员可以轻松地实现调用时才装载的的方式。这样可以有效地降低调试的时间和重新启动服务器的时间。
 * </p>
 * 
 ...
 * @since CFDK 1.0
 */
public class CosmosDbGlobalCommandLoader extends
		AbstractCosmosGlobalCommandLoader {

	/**
	 * 装载所有根命令的SQL语句。
	 */
	private static final String ALL_ROOT_COMMANDS = "SELECT command.ID, command.NAME, command.DOMAIN, command.COMMAND, command.EXECUTER, command.TYPE, command.REMARK, command.DEBUG_LEVEL,"
			+ " arg.NAME, arg.CONVERTER, arg.IN_OUT_TYPE, arg.REMARK, composite.LFT_INDEX, composite.RGT_INDEX"
			+ " FROM tb_cos_composite_command composite"
			+ " LEFT JOIN tb_cos_command command ON command.DOMAIN = composite.DOMAIN AND command.NAME = composite.COMMAND"
			+ " LEFT JOIN tb_cos_command_arg arg ON arg.DOMAIN = command.DOMAIN AND arg.COMMAND = command.NAME"
			+ " WHERE composite.LFT_INDEX = 1 ORDER BY command.DOMAIN ASC, command.NAME ASC, arg.ARG_INDEX ASC";

	/**
	 * 装载某个域下所有根命令的SQL语句。
	 */
	private static final String DOMAIN_ROOT_COMMANDS = "SELECT command.ID, command.NAME, command.DOMAIN, command.COMMAND, command.EXECUTER, command.TYPE, command.REMARK, command.DEBUG_LEVEL,"
			+ " arg.NAME, arg.CONVERTER, arg.IN_OUT_TYPE, arg.REMARK, composite.LFT_INDEX, composite.RGT_INDEX"
			+ " FROM tb_cos_composite_command composite"
			+ " LEFT JOIN tb_cos_command command ON command.DOMAIN = composite.DOMAIN AND command.NAME = composite.COMMAND"
			+ " LEFT JOIN tb_cos_command_arg arg ON arg.DOMAIN = command.DOMAIN AND arg.COMMAND = command.NAME"
			+ " WHERE command.DOMAIN = ? AND composite.LFT_INDEX = 1 ORDER BY command.DOMAIN ASC, command.NAME ASC, arg.ARG_INDEX ASC";

	/**
	 * 装载单个命令的SQL语句。
	 */
	private static final String SINGLE_COMMAND = "SELECT command.ID, command.NAME, command.DOMAIN, command.COMMAND, command.EXECUTER, command.TYPE, command.REMARK, command.DEBUG_LEVEL,"
			+ " arg.NAME, arg.CONVERTER, arg.IN_OUT_TYPE, arg.REMARK, composite.LFT_INDEX, composite.RGT_INDEX"
			+ " FROM tb_cos_composite_command composite"
			+ " LEFT JOIN tb_cos_command command ON command.DOMAIN = composite.DOMAIN AND command.NAME = composite.COMMAND"
			+ " LEFT JOIN tb_cos_command_arg arg ON arg.DOMAIN = command.DOMAIN AND arg.COMMAND = command.NAME"
			+ " WHERE command.NAME = ? AND command.DOMAIN = ? AND composite.LFT_INDEX = 1 ORDER BY command.DOMAIN ASC, command.NAME ASC, arg.ARG_INDEX ASC";

	/**
	 * 深度装载某个命令下所有子命令的SQL语句。
	 */
	private static final String ALL_SUB_COMMANDS = "SELECT command.ID, composite.COMMAND, composite.DOMAIN, command.COMMAND, command.EXECUTER, command.TYPE, command.REMARK, command.DEBUG_LEVEL,"
			+ " arg.NAME, arg.CONVERTER, arg.IN_OUT_TYPE, arg.REMARK, composite.LFT_INDEX, composite.RGT_INDEX"
			+ " FROM tb_cos_composite_command composite"
			+ " LEFT JOIN tb_cos_command command ON command.DOMAIN = composite.DOMAIN AND command.NAME = composite.COMMAND"
			+ " LEFT JOIN tb_cos_command_arg arg ON arg.DOMAIN = command.DOMAIN AND arg.COMMAND = command.NAME"
			+ " WHERE composite.COMPOSITE = ? AND composite.LFT_INDEX > ? AND composite.LFT_INDEX < ? ORDER BY composite.LFT_INDEX ASC, command.DOMAIN ASC, command.NAME ASC";
	 ...
	 *            所属域。
	 */
	public CosmosDbGlobalCommandLoader(CosmosFactory factory,
			CosmosDomain domain) {
	...
	protected CosmosCompositeCommand deepLoadCommand(
		...
		for (CosmosCommand cosmosCommand : loadedCommands) {
			loadedCommandMap.put(cosmosCommand.getDomain().getName() + "."
					+ cosmosCommand.getName(), cosmosCommand);
		}
		...
		try {
			Map<String, CosmosMetaCommand> commandTypes = config
					.getCommandTypes();
			String composite = command.getDomain().getName() + "."
					+ command.getName();
			...
			while (rs != null && rs.next()) {
				...
				if ((id != null && !id.equals(currentCommand)) || id == null) {
					if (id != null && !id.equals(currentCommand)
							|| (id == null && currentCommand != id)) {
						if (currentCommand != null) {
							...
							CosmosDomain cosmosDomain = config.getDomains()
									.get(domain);
							...
							CosmosCommand cosmosCommand = constructor
									.newInstance(name, cosmosDomain, content,
											executer, metaArgs, commandType,
											CosmosCommandSource.GLOBAL);
							...
							if (debugLevel == null
									|| "".equals(debugLevel.trim())
									|| DebugLevel.NO_DEBUG.toString().equals(
											debugLevel)) {
								cosmosCommand.setDebugLevel(cosmosDomain
										.getDebugLevel());
							} else {
							...
							if (stack.size() > 0) {
								...
								do {
									...
									if (priorCommand instanceof CosmosCompositeCommand) {
										...
										|| parentCommand.getRightIndex() < rightIndex);
					...
					if (id == null) {
						...
						leftIndex = rs.getLong(13);
						rightIndex = rs.getLong(14);
						CosmosCommand cosmosCommand = loadedCommandMap
								.get(domain + "." + name);
						if (cosmosCommand == null) {
							throw new CosmosCommandLoaderException(domain + "."
									+ name + " doesn't exist.");
						}
				...
				if (argName != null) {
					...
					CosmosMetaArgument arg = new StandardCosmosMetaArgument(
							this.factory.getConverter(converter), argName,
							inOutType == null ? ArgumentInOutType.IN
									: ArgumentInOutType.valueOf(inOutType),
							argRemark);
					metaArgs.add(arg);
				}
			}
			...
			if (currentCommand != null) {
				...
				Constructor<CosmosCommand> constructor = commandType.getClazz()
						.getConstructor(String.class, CosmosDomain.class,
								String.class, String.class, Collection.class,
								CosmosMetaCommand.class,
								CosmosCommandSource.class);

				CosmosCommand cosmosCommand = constructor.newInstance(name,
						cosmosDomain, content, executer, metaArgs, commandType,
						CosmosCommandSource.GLOBAL);
				...
				cosmosCommand.setRemark(remark);
				if (debugLevel == null || "".equals(debugLevel.trim())
						|| DebugLevel.NO_DEBUG.toString().equals(debugLevel)) {
	...
	protected Collection<CosmosCommand> loadDomainRootCommands(
		...
		try {
				...
				if (!id.equals(currentId)) {
					if (currentId != null) {
						...
						CosmosCommand cosmosCommand = constructor.newInstance(
								...
								commandType, CosmosCommandSource.GLOBAL);
			...
			if (id != null) {
				...
				CosmosCommand cosmosCommand = constructor.newInstance(name,
						domain, command, executer, metaArgs, commandType,
						CosmosCommandSource.GLOBAL);
	...
	protected Collection<CosmosCommand> loadRootCommands()
		...
		try {
			...
			while (rs != null && rs.next()) {
				...
				if (!id.equals(currentId)) {
					if (currentId != null) {
						...
						CosmosCommand cosmosCommand = constructor.newInstance(
								name, cosmosDomain, command, executer,
								metaArgs, commandType,
								CosmosCommandSource.GLOBAL);
			...
			if (id != null) {
				...
				CosmosCommand cosmosCommand = constructor.newInstance(name,
						cosmosDomain, command, executer, metaArgs, commandType,
						CosmosCommandSource.GLOBAL);
